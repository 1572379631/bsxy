          ...
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
            <thead>
              <tr class="am-success">
                ...
                <th class="table-date am-hide-sm-only">审批结果</th>
                <th width="130px" class="table-set">详细</th>
              </tr>
            </thead>
            <tbody>
            	<c:forEach items="${listPlaceInfo}" var="list">
	              <tr>
	             <td><a href="javasripit:;" onclick="queryApplyPerson(${list.postulantId})" class="am-text-secondary" title="查看审批人信息">${list.postulantName}</a></td>
              ...
              <ul class="am-pagination am-fr">
									<c:if test="${page.currnav>1}">
										<li><a href="${pageContext.request.contextPath}/query/approvadNot?flagb=1&currnav=${page.first}">首页</a></li>
									</c:if>
									<c:if test="${page.currnav>1}">
										<li><a href="${pageContext.request.contextPath}/query/approvadNot?flagb=1&currnav=${page.currnav-1}">«</a></li>
									</c:if>
									<c:forEach begin="${page.begin }" end="${page.end }" var="num">
									<c:choose>
									<c:when test="${page.currnav==num }">
										<li class="am-active"><a href="${pageContext.request.contextPath}/query/approvadNot?flagb=1&currnav=${num}">${num}</a></li>
									</c:when>
									<c:otherwise>
										<li><a href="${pageContext.request.contextPath}/query/approvadNot?flagb=1&currnav=${num}">${num}</a></li>
									</c:otherwise>
									</c:choose>
									</c:forEach>
									...
									<c:if test="${page.currnav< page.last }">
										<li><a href="${pageContext.request.contextPath}/query/approvadNot?flagb=1&currnav=${page.end}">尾页</a></li>
									</c:if>
								</ul>
